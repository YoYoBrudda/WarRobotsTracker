...
def estimate_ally_table_geometry(image: Image.Image) -> Tuple[float, float, float]:
    ...
    try:
        ...
        row_h_px = max(int(header_h_px * 1.18), int(w * 0.036), int(h * 0.060))
...
def ocr_scoreboard_row(row_crop: Image.Image) -> str:
    """OCR a single ally row as one line.

    V5 originally OCRed tiny cells one at a time. That worked for large values like
    honor/damage, but Tesseract often missed the tiny 0/1/2 stat cells. OCRing the
    row as one line gives Tesseract more context and is much more reliable.
    """
    row = ImageOps.grayscale(row_crop)
    row = row.resize((row.width * 3, row.height * 3))
    row = ImageEnhance.Contrast(row).enhance(3.0)
    row = ImageEnhance.Sharpness(row).enhance(2.0)
    texts = []
    for psm in [6, 7, 11]:
        try:
            texts.append(pytesseract.image_to_string(row, config=f"--psm {psm}").strip())
        except Exception:
            pass
    # Prefer the line with the most digit groups because that usually contains the stats.
    return max(texts, key=lambda t: len(re.findall(r"\d+", t)), default="")
...
def parse_by_adaptive_layout(image_path: Path, player_name: str = "", manual_place: Optional[int] = None, full_text: str = "") -> Dict[str, str]:
    ...
    # Crop only the numeric section of the target ally row.
    row_crop = image.crop(cell_bbox(w, h, 0.150, y1, 0.470, y2))
