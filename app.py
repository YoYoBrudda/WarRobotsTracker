...
def estimate_ally_table_x_bounds(image: Image.Image, rows_top: float) -> Tuple[float, float]:
    ...
    V16 uses the blue header background itself, not the white OCR icons, so black
...
def parse_stats_from_cells(image: Image.Image, place: int, rows_top: float, row_h: float) -> Dict[str, str]:
    ...
    table_w = x2r - x1r
    # Slightly expand vertically because War Robots digits sit low in each row,
    # especially on YouTube/fullscreen captures.
...
def estimate_ally_table_geometry(image: Image.Image) -> Tuple[float, float, float]:
    ...
    try:
        ...
        # V16 fix: row height must be based on the visible scoreboard/header height,
...
def ocr_scoreboard_row(row_crop: Image.Image) -> str:
    ...
    V16 keeps this deliberately simple. The previous heavy thresholding sometimes
...
def parse_by_adaptive_layout(image_path: Path, player_name: str = "", manual_place: Optional[int] = None, full_text: str = "") -> Dict[str, str]:
    ...
    # V16: crop the highlighted/player stat row using detected table bounds, then
    # parse the row as a single stat line. This avoids the old width-based row-shift
    # bug while staying much faster than OCRing every cell first.
    y1 = rows_top + (place - 1) * row_h - row_h * 0.02
    y2 = rows_top + place * row_h + row_h * 0.15
    x1r, x2r = estimate_ally_table_x_bounds(image, rows_top)
    table_w = x2r - x1r
...
class TrackerApp(tk.Tk):
    ...
    def create_widgets(self):
        pad = {"padx": 12, "pady": 6}
        title = ttk.Label(self, text=f"{APP_NAME} - Version 16", font=("Arial", 18, "bold"))
        ...
        bottom = ttk.Label(self, text="V16: Portable EXE storage + improved stat parsing. Folders are created beside the app by default, but you can choose another folder.")
