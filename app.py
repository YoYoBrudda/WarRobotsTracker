"""
War Robots Screenshot Tracker - Version 17
...
def estimate_ally_table_x_bounds(image: Image.Image, rows_top: float) -> Tuple[float, float]:
    ...
    V17 uses the blue header background itself, not the white OCR icons, so black
...
def estimate_ally_table_geometry(image: Image.Image) -> Tuple[float, float, float]:
    ...
    try:
        ...
        # V17 fix: row height must be based on the visible scoreboard/header height,
...
def ocr_scoreboard_row(row_crop: Image.Image) -> str:
    ...
    V17 keeps this deliberately simple. The previous heavy thresholding sometimes
...
def parse_by_adaptive_layout(image_path: Path, player_name: str = "", manual_place: Optional[int] = None, full_text: str = "") -> Dict[str, str]:
    ...
    V13 normal processing does NOT use the manual row override. Manual row is only
    for the Learn Name button. For normal screenshots, this parser:
    1) tries to find the saved player name on the ally side,
    2) otherwise falls back to YOUR PLACE ON THE TEAM,
    3) refuses plain English/numeric names that are not found, so wrong-player
       screenshots do not get logged as bad data.
    """
    image = Image.open(image_path).convert("RGB")
    w, h = image.size
    ...
    # V13: first read the explicit top banner place. This is more reliable than
    # fuzzy username matching for low-quality screenshots and symbol-heavy names.
    ...
    # First choice: find the configured player name directly in the ally table.
    if player_name:
        try:
            ...
            if row_y is not None:
                ...
                if guessed is not None:
                    name_found = True
                    # If the banner gives a clear row, trust it. Fuzzy name matching
                    # can jump rows when OCR partially matches a title or clan tag.
                    place = banner_place if banner_place is not None else guessed
                    source = "place_banner" if banner_place is not None else "name"
        except Exception:
            pass

    # If the name was not found, only use the banner row when the filename or OCR
    # text still supports the saved player name. This fixes good files like
    # Wolfblood_003.png while still sending unrelated screenshots to needs_review.
    ...
    # Fallback for symbol-heavy/non-English names or blank names: use top banner.
    if place is None:
        place = banner_place
    ...
    # V17: use the visible highlighted player row for the actual stat crop.
    # This fixes shifted/wide screenshots where OCR/banner place is correct but
    # percentage row math lands between two rows.
    ...
    x1r, x2r = estimate_ally_table_x_bounds(image, rows_top)
    table_w = x2r - x1r
    row_crop = image.crop(cell_bbox(w, h, x1r + table_w * 0.40, y1, x1r + table_w * 0.92, y2))
    row_text = ocr_scoreboard_row(row_crop)
    parsed = parse_numbers_from_row_text(row_text)

    # Cell OCR is now only a fallback for incomplete rows, since it is slower.
    if not (parsed.get("honor_points") and parsed.get("damage_healing") and parsed.get("assists") and parsed.get("kills")):
        if highlighted is not None:
            # Temporarily infer a synthetic row position from the highlighted bounds
            # so cell crops use the exact highlighted row, not the old row math.
            y_mid = (y1 + y2) / 2.0
            synthetic_top = y1
            synthetic_row_h = max(0.001, y2 - y1)
            cell_parsed = parse_stats_from_cells(image, 1, synthetic_top, synthetic_row_h)
        else:
            cell_parsed = parse_stats_from_cells(image, place, rows_top, row_h)
        for key in ["honor_points", "damage_healing", "assists", "kills", "beacons"]:
            if (not parsed.get(key) or parsed.get(key) == "N/A") and cell_parsed.get(key):
...
class TrackerApp(tk.Tk):
    ...
    def create_widgets(self):
        pad = {"padx": 12, "pady": 6}
        title = ttk.Label(self, text=f"{APP_NAME} - Version 17", font=("Arial", 18, "bold"))
        ...
        bottom = ttk.Label(self, text="V17: Portable EXE storage + improved stat parsing. Folders are created beside the app by default, but you can choose another folder.")
